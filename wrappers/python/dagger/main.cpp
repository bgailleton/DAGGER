...
void declare_graph(py::module &m, std::string typestr) {
  ...
  py::class_<graph<double, CONNECTOR_T>>(m, typestr.c_str())
      .def(py::init<int, int>())
      .def("get_DA_proposlope",
           &graph<double, CONNECTOR_T>::template get_DA_proposlope<
               CONNECTOR_T, std::vector<double>, py::array>)
      .def("get_DA_SS", &graph<double, CONNECTOR_T>::template get_DA_SS<
                            CONNECTOR_T, std::vector<double>, py::array>)
      .def("get_rowcol_Sreceivers",
           &graph<double,
                  CONNECTOR_T>::template get_rowcol_Sreceivers<CONNECTOR_T>)
      .def("print_receivers",
           &graph<double, CONNECTOR_T>::template print_receivers<
               CONNECTOR_T, std::vector<double>>)
      .def("get_rec_array_size",
           &graph<double, CONNECTOR_T>::get_rec_array_size)
      .def("init_graph",
           &graph<double, CONNECTOR_T>::template init_graph<CONNECTOR_T>)
      .def("update_Mrecs", &graph<double, CONNECTOR_T>::template update_Mrecs<
                               CONNECTOR_T, std::vector<double>>)
      .def("update_recs", &graph<double, CONNECTOR_T>::template update_recs<
                              CONNECTOR_T, std::vector<double>>)
      .def("set_opt_stst_rerouting",
           &graph<double, CONNECTOR_T>::set_opt_stst_rerouting)
      .def("compute_graph", &graph<double, CONNECTOR_T>::template compute_graph<
                                CONNECTOR_T, py::array_t<double, 1>, py::array>)
      .def("compute_graph_timer",
           &graph<double, CONNECTOR_T>::template compute_graph_timer<
               CONNECTOR_T, py::array_t<double, 1>, py::array>)
      .def("test_Srecs",
           &graph<double, CONNECTOR_T>::template test_Srecs<py::array>)
      .def("sum_at_outlets",
           &graph<double, CONNECTOR_T>::template sum_at_outlets<
               CONNECTOR_T, py::array_t<double, 1>, double>)
      .def("keep_only_at_outlets",
           &graph<double, CONNECTOR_T>::template keep_only_at_outlets<
               CONNECTOR_T, py::array_t<double, 1>, py::array>)
      ...
      .def("get_SFD_dx",
           &graph<double,
                  CONNECTOR_T>::template get_SFD_dx<py::array_t<double, 1>>)
      .def("get_SFD_ndonors",
           &graph<double,
                  CONNECTOR_T>::template get_SFD_ndonors<py::array_t<int, 1>>)
      .def("get_SFD_donors_flat",
           &graph<double, CONNECTOR_T>::template get_SFD_donors_flat<
               py::array_t<int, 1>>)
      ...
      .def("get_linknodes_flat_D4",
           &graph<double, CONNECTOR_T>::template get_linknodes_flat_D4<
               py::array_t<int, 1>>)
      .def("get_linkdx_flat_D4",
           &graph<double, CONNECTOR_T>::template get_linkdx_flat_D4<
               py::array_t<double, 1>, CONNECTOR_T>)
      ...
      .def("get_SFD_dx_at_node",
           &graph<double, CONNECTOR_T>::get_SFD_dx_at_node)
      .def("get_SFD_ndonors_at_node",
           &graph<double, CONNECTOR_T>::get_SFD_ndonors_at_node)
      .def("get_SFD_donors_at_node",
           &graph<double, CONNECTOR_T>::template get_SFD_donors_at_node<
               std::vector<int>>)
      .def("get_SFD_gradient",
           &graph<double, CONNECTOR_T>::template get_SFD_gradient<
               py::array_t<double, 1>, py::array_t<double, 1>>)
      .def("get_links_gradient",
           &graph<double, CONNECTOR_T>::template get_links_gradient<
               CONNECTOR_T, py::array_t<double, 1>, py::array_t<double, 1>>)
      ...
      .def("get_SFD_basin_labels",
           &graph<double, CONNECTOR_T>::
               template get_MFD_min_distance_from_outlets<CONNECTOR_T,
                                                          py::array_t<int, 1>>)
...
PYBIND11_MODULE(dagger, m) {
  ...
  m.def("hillshade", &hillshade<D8connector<double>, py::array_t<double, 1>,
