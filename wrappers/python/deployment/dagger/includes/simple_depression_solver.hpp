...
out_t
label_depressions_PQ(Connector_t* connector, in_t& ttopography)
{
...
std::vector<int>
_label_depressions_PQ(Connector_t* connector,
											in_t& topography,
											int& ndep,
											DBBTree& tree)
{
...
template<class in_t, class out_t, class Connector_t>
out_t
label_ocean(Connector_t* connector, in_t& ttopography)
{
...
template<class in_t, class Connector_t>
std::vector<int>
_label_ocean(Connector_t* connector, in_t& topography)
{
	// TEST LABELLING HERE
...
_dagger_fill(Connector_t* connector,
						 topo_t& topography,
						 std::vector<size_t>& stack)
{
	...
	int npits = 0;
	for (int i = 0; i < connector->nnodes; ++i) {
		int nn = connector->get_neighbour_idx_nodes_and_links(
			i, neighbours, neighbours_links);
		trackneighb[i] += nn;
	...
	while (divide_PQ.empty() == false) {
		...
		while (phil_collins.empty() == false) {
			...
			int nn = connector->get_neighbour_idx_nodes_and_links(
				next, neighbours, neighbours_links);
...
template<class in_t, class Connector_t>
std::vector<int>
_yolo45(Connector_t* connector, in_t& topography)
{
...
bool
simple_depression_solver(Connector_t* connector,
