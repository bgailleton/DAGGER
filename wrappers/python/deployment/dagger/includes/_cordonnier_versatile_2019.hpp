...
template<class float_t>
inline bool
operator>(const _basinLink<float_t>& lhs, const _basinLink<float_t>& rhs)
{
...
public:
					 ...
					 std::vector<size_t>& Sstack)
	{
		...
		for (int i = 0; i < int(connector->linknodes.size()); ++i) {
			...
			_basinLink<float_t> tbasl(
				bj, bk, connector->linknodes[j], connector->linknodes[k], score);
