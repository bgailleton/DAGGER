...
out_t
standalone_priority_flood(int_t& topography, Connector_t& connector)
{
...
void
PriorityFlood(std::vector<fT>& topography, Connector_t& connector)
{
	PQFORPF open;
	std::queue<PQH> pit;
									...
									fT step)
{
	/*
		...
		has normal boundaries (-> 4edges and no nodata) Implementation adapted from
		RichDEM
	*/

	// the priority queue sorting the data on the go
	std::priority_queue<PQ_helper2D<int, fT>,
											std::vector<PQ_helper2D<int, fT>>,
											std::greater<PQ_helper2D<int, fT>>>
		open;

	// Checking which depression is already closed
	std::vector<int8_t> closed(topography.size(), false);
	std::queue<PQ_helper2D<int, fT>> pit;

	// First pass adding edges to the PQ
	for (size_t r = 0; r < grid.ny; ++r) {
		for (size_t c = 0; c < grid.nx; ++c) {

			// normal BCs so all borders are out edges
			if (grid.can_out(r, c, BCs, &grid)) {
				// vectorised index
				int i = r * grid.nx + c;
				// Is in queue and processed
				open.emplace(PQ_helper2D<int, fT>(r, c, topography(r, c)));
				closed[i] = true;
			}
		}
	}

	// Helper with random number generation
	auto randu = easyRand();

	// cell placeholder
	PQ_helper2D<int, fT> cell;

	// Algorithm is running as long as I have nodes Priority Queue
	fT PitTop = -9999;
	while (open.size() > 0 || pit.size() > 0) {

		// Popping the next lowest cell
		...
		if (pit.size() > 0 && open.size() > 0 &&
			...
			pit.pop();
			if (PitTop == -9999)
				PitTop = topography(cell.row, cell.col);
		} else {
			cell = open.top();
			open.pop();
			PitTop = -9999;
		...
		fT ttopo = topography(r, c);

		// traversing neighbours
		for (int ri = -1; ri < 2; ++ri) {
			for (int ci = -1; ci < 2; ++ci) {

				// if ri == ci I am either Diagonal (not D4) or the current node (0,0)
				if (abs(ri) == abs(ci))
					continue;
				...
				// Edge check
				if (rn < 0 || rn > grid.ny - 1 || cn < 0 || cn > grid.nx - 1)
					continue;

				if (grid.can_give(rn, cn, BCs, &grid) == false)
					continue;

				// vectorised ID
				int n = rn * grid.nx + cn;

				// if already processed -> not interested
				if (closed[n])
					continue;
				...
				if (topography(rn, cn) <= ttopo + step) {
					...
					topography(rn, cn) = tttopo;
					pit.emplace(rn, cn, topography(rn, cn));

				} else {
					// to be processed: emplacing in the PQ
					open.emplace(rn, cn, topography(rn, cn));
				}
			}
		}
	}
...
template<class fT, class Connector_t>
void
_PriorityFool(std::vector<fT>& topography,
...
void
morphologicalReconstruction(std::vector<double>& data, int width, int height)
{
...
out_t
standalone_priority_flood_opti(int_t& topography,
															 Connector_t& connector,
															 graph_t& GRAPH)
{
	auto ttopo = format_input(topography);
...
template<class fT, class Connector_t>
bool
PriorityFlood_opti(std::vector<fT>& topography,
									 Connector_t& connector,
									 std::vector<size_t>& Sstack)
{
	PQFORPF open;
...
std::vector<double>
PriorityFlood_old(topo_t& ttopography, Connector_t& connector)
{
...
std::vector<double>
PriorityFlood_broken(topo_t& ttopography,
										 Connector_t& connector,
										 Graph_t& graph)
{
	std::random_device rd;	// obtain a random number from hardware
