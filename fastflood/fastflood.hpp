...
public:
	...
	void run_MFD_dynamic()
	{
		...
		for (int i = this->graph->nnodes - 1; i >= 0; --i) {
			...
			for (int ttl = 0; ttl < nn; ++ttl) {
				...
				// ### Storing local slope to the power needed
				sqrtgrads[ttl] =
					(parting_coeff == 0.5) ? tsqrt : std::pow(tgrad, this->parting_coeff);
				// ### SUmming the slope to the power needed
				sumgrad_sqrt += sqrtgrads[ttl];
				// ### accumulating the equation factor
				facgrad += tgrad / dx;
	...
	void _get_w_eff(std::vector<float_t>& w_eff,
									float_t prec,
	...
	void _get_a_eff(std::vector<float_t>& a_eff,
									float_t prec,
									bool recompute_graph)
	{
	...
	void run_SFD_with_erosion(float_t ke,
														float_t aexp,
														float_t tau_c,
														float_t transport_length,
														float_t ke_lat,
														float_t kd_lat)
	{
		...
		for (int i = this->graph->nnodes - 1; i >= 0; --i) {
			...
			if (this->depth_limiter && thw > this->depth_threshold)
				regulator *=
					std::pow(this->depth_threshold / thw, this->sensibility_to_flowdepth);
	...
	void run_MFD_erosion(float_t ke,
											 float_t aexp,
											 float_t tau_c,
											 float_t transport_length,
											 float_t ke_lat,
	...
	void run_MFD_erosion_B(float_t ke,
												 float_t aexp,
												 float_t tau_c,
												 float_t transport_length,
												 float_t ke_lat,
			...
			for (int j = 0; j < nr; ++j) {
				...
				if (true) {
					...
					if (std::isfinite(
								...
								tQin) == false) {
						std::cout << fac *
														 (edot + latedotB + latddotA - d_dot - latddotA -
															latddotB) *
														 cA +
													 tQin
											<< std::endl;
	...
	void run_MFD_static(float_t ke,
											float_t aexp,
											float_t tau_c,
											float_t transport_length,
											float_t ke_lat,
											...
											int n_erosion,
											float_t dt_erosion,
											bool stochastic_slope,
											bool no_erosion_at_all)
	{
		bool erosion = false;
		this->stochaslope = 0;
	...
	void run_MFD_static_SPL(float_t Ker,
													float_t Kerl,
													float_t Kdepl,
													float_t mexp,
													float_t nexp,
		...
		for (int i_prec = 0; i_prec < n_prec; ++i_prec) {
			...
			while (!this->connector->flow_out_model(i)) {
				...
				for (int tn = 0; tn < nl; ++tn) {
					...
					this->hw[ri] =
						this->hw[ri] *
						std::pow(1.0 + W_coef * std::pow(this->hw[ri], MANNING_COEF) * tdt,
										 -MANNING_COEF_INV);
