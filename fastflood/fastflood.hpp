...
public:
	...
	std::vector<float_t> hw, weights, Qwin, Qwout, Qsin, Qsout, qs, Qlink, lastdt, lastdl, lastS;
	...
	fastflood(Graph_t& graph, Connector_t& connector, topo_t& topo, topo_t& Qbase)
	{
	...
	template<class in_t>
	void spatial_dt(in_t& DTS){this->spatially_varying_dt = true;auto tdts =  DAGGER::format_input(DTS); this->dts = DAGGER::to_vec(tdts);}
	void set_dt(float_t dt){this->spatially_varying_dt = false;this->dts = {dt};}
	...
	void set_out_boundaries_to_permissive(){this->connector->set_out_boundaries_to_permissive();}
	void set_edges_to_0()
	{
		for(int i = 0; i<this->graph->nnodes; ++i)
		{
	...
	void set_sensibility_to_flowdepth(float_t exp){this->sensibility_to_flowdepth = exp;}
	float_t get_sensibility_to_flowdepth(){return this->sensibility_to_flowdepth;}
	...
	// only_SS determines if the graph only needs Steepest Descent (true) or multiple flow (false)
	...
	void run_SFD()
	{
		...
		// std::cout << "In: " << totqin << " out: " << totqinout << " Balance::" << totqin - totqinout << std::endl; 
		// std::cout << "man::" << 1/mannings << " hwpow::" << sumHW << " grad::" << sumgrad_sqrt << " sumgrad2 " << sumgrad2 << " nmneg = " << nneg<< std::endl;
		// std::cout << "DEBUGDEBUGA5" << std::endl;
	...
	void run_MFD()
	{
		...
		for(int i=0; i<this->connector->nnodes; ++i)
		{
			...
			{
				this->hw[i] += surfpp[i] - surf[i];
				// Tracking the number of pixel filled as one indicator for stability
				n_pixel_filled++;
			}
		}
		if(this->REPORT_N_PIXEL_FILLED)
		...
		for(int i = this->graph->nnodes-1; i>=0; --i)
		{
			...
			//## bunch of temporary variables to accumulate factors, reuse characteristics and determine max slope
			float_t maxgrad_sqrt = 0, sumgrad_sqrt = 0, facgrad = 0, cellarea = this->connector->get_area_at_node(i);
			...
			for(int ttl = 0; ttl<nn; ++ttl)
			{
				...
				sqrtgrads[ttl] = (parting_coeff == 0.5)?tsqrt:std::pow(tgrad, this->parting_coeff);
				sqrtgrads[ttl] *= 1 - this->randu.get() * this->stochaslope;
				...
				if(this->hflow)
				{
					...
					facgrad += std::pow(this->get_hflow_at_link(reclink[ttl]), FIVETHIRD) * tgrad/dx;
				}
				else
					facgrad += tgrad/dx;
			...
			if(this->hflow)
				this->Qwout[node] = this->topological_number * facgrad/maxgrad_sqrt * cellarea/this->mannings;// * (1/(4 * this->connector->dxy )+ 1/ (2*this->connector->dx) + 1/(2 * this->connector->dy));
			else
		...
		for(int n_opti =0; n_opti < n_run_by_graph_cycle; ++n_opti)
		{
			...
			// std::cout << "DEBUGDEBUGA5" << std::endl;
				...
				if(sumgrad_sqrt > 0)
				{
					...
					this->Qwout[node] = facgrad/maxgrad_sqrt * std::pow(this->hw[node],5./3) * 0.5/this->mannings;
				}
				else
				{
	...
	void _get_w_eff(std::vector<float_t>& w_eff, float_t prec, bool recompute_graph)
	{
		...
		for(int i=0; i< this->connector->nnodes; ++i)
		{
			if(this->graph->is_active(i, *this->connector))
			{
				w_eff[i] = A[i]/a_eff[i];
			}
		}
	...
	template<class out_t>
	out_t get_a_eff(float_t prec, bool recompute_graph)
	{
		std::vector<float_t> a_eff(this->graph->nnodes, 0.);
		this->_get_a_eff(a_eff,prec, recompute_graph);
	...
	void _get_a_eff(std::vector<float_t>& a_eff, float_t prec, bool recompute_graph)
	{
	...
	template<class out_t>
	out_t get_hydraulic_slope_D8(bool recompute_graph)
	{
	...
	void _get_hydraulic_slope_D8(std::vector<float_t>& S_h, bool recompute_graph)
	{
	...
	// For the physics see paper from Bates et al., 2010 and the implementation is adapted and modified from HAIL-CAESAR implementation by Declan Valters
	float_t caesar_lisflood()
	{
		...
		float_t maxhw = std::max(*std::max_element(this->hw.begin(),this->hw.end()), 1e-3);

		float_t tdt = this->connector->dxmin * this->alpha/std::sqrt(GRAVITY * maxhw);
		...
		for(size_t i=0; i < this->Qlink.size(); ++i)
		{
			...
			float_t Sw = (surf[n1] - surf[n2])/this->connector->get_dx_from_links_idx(i);

			// Getting hflow, the flow height between the two cell:
			float_t hflow = std::max(surf[n1], surf[n2]) - std::max(this->topography[n1], this->topography[n2]);
			...
			float_t tdx = this->connector->get_traverse_dx_from_links_idx(i) * this->topological_number;

			// Get the discharge
			this->Qlink[i] = tdx * (this->Qlink[i] - GRAVITY * hflow * tdt * Sw) / ( 1 + (GRAVITY * hflow * tdt * std::pow(this->mannings,2) * std::abs(this->Qlink[i])/std::pow(hflow,10./3.) ) );

			// // Discahrge checkers
			// if(std::abs(this->Qlink[i])/hflow / std::sqrt(GRAVITY * hflow) > this->froude_number )
			// 	this->Qlink[i] = DAGGER::sgn(this->Qlink[i]) * hflow * std::sqrt(GRAVITY * hflow) * this->froude_number;

			// if(std::abs(this->Qlink[i]) * tdt / tdx > this->hw[n2]/4 && DAGGER::sgn(this->Qlink[i]) > 0 )
			// 	this->Qlink[i] = this->hw[n2] * this->connector->get_area_at_node(n2)/5/tdt;

			// else if (std::abs(this->Qlink[i]) * tdt / tdx >  this->hw[n1]/4 && DAGGER::sgn(this->Qlink[i]) < 0 )
			// 	this->Qlink[i] = -1 * this->hw[n1] * this->connector->get_area_at_node(n1)/5/tdt;
	...
	// Simply copy Qbase in Qwin in order to have something for the first iteration of the nograph version
	void init_Qwin_with_Qbase(){for(int i=0; i<this->graph->nnodes;++i)this->Qwin[i] = this->Qbase[i];}
	void increment_hw_from_Qbase(float tdt){for(int i=0; i<this->graph->nnodes;++i)this->hw[i] += tdt * this->Qbase[i]/this->connector->get_area_at_node(i);}
	...
	template<class out_t>
	out_t get_hw(){ return DAGGER::format_output<std::vector<float_t>, out_t >(this->hw) ;}
	template<class out_t>
	out_t get_Qwin(){ return DAGGER::format_output<std::vector<float_t>, out_t >(this->Qwin) ;}
	template<class out_t>
	out_t get_Qs(){ return DAGGER::format_output<std::vector<float_t>, out_t >(this->Qsin) ;}
	template<class out_t>
	out_t get_Qwout(){ return DAGGER::format_output<std::vector<float_t>, out_t >(this->Qwout) ;}
	template<class out_t>
	out_t get_topography(){ return DAGGER::format_output<std::vector<float_t>, out_t >(this->topography) ;}

	template<class out_t>
	out_t get_spatial_dts(){ return DAGGER::format_output<std::vector<float_t>, out_t >(this->saved_dts) ;}

	template<class topo_t>
	void set_Qbase(topo_t& Qbase)
	{
	...
	void init_erosion()
	{
		...
		for(size_t i=0;i<this->Qs_entry_point.size(); ++i) this->Qsin[this->Qs_entry_point[i]] = this->Qs_entry[i];
	}
	...
	void force_entry_point_above()
	{
		...
		for(size_t i=0;i<this->Qs_entry_point.size(); ++i)
		{
			...
			{
				int onode = recidx[j];
				if(this->topography[node] + this->hw[node] < this->topography[onode] + this->hw[onode])
				{
	...
	void run_SFD_with_erosion(float_t ke, float_t aexp, float_t tau_c, float_t transport_length, float_t ke_lat, float_t kd_lat)
	{
		...
		for(size_t i=0;i<this->Qs_entry_point.size(); ++i) 
		{
			// std::cout << this->Qs_entry_point[i] << " gets " << this->Qs_entry[i] << std::endl;
		...
		for(int i = this->graph->nnodes - 1; i>=0; --i)
		{
			...
			if(this->graph->is_active(node, *this->connector) == false || this->hw[node] <= 0)
				continue;
			...
			float_t regulator = 1.;
			// if(thw > 1) thw = 1;
			// if hflow is activated, I check how disconnected it is from the next node
			if(thf < this->hw[node]) regulator = std::pow(thf/this->hw[node], this->sensibility_to_flowdepth);
			// if depth limiter is activated, I reduce erosion after a certain depth, balancing the effect of over carving when 
			if(this->depth_limiter && thw > this->depth_threshold) regulator *= std::pow(this->depth_threshold/thw, this->sensibility_to_flowdepth);

			// if(regulator )

			// Getting the hydraulic slope
			float_t Sw = std::max(this->topography[node]+this->hw[node] - this->topography[rec]-this->hw[rec], 1e-6) / tdx;
			...
			// 	std::cout << "S calc :: " << this->topography[node]<< " + " << this->hw[node] << " - " << this->topography[rec] << " - " << this->hw[rec] << " Node - rec were " << node << " and " << rec << std::endl;
			// }

			// rinit all to 0
			float_t edot = 0,ddot = 0,latedotA = 0,latedotB = 0, latddotA = 0, latddotB = 0;


			// if tau exceed the critical motion threshold: calculating edot
			if(tau>tau_c) edot = ke * std::pow(tau - tau_c,aexp);
			...
			if(orthonodes.first >= 0 && orthonodes.first < this->connector->nnodes)
			{
				...
				if(this->topography[to] > this->topography[node]) latedotA = (this->topography[to] - this->topography[node])/tdw * ke_lat * edot;
				// if lateral node has lower ground: I deposit
				if(this->topography[to] < this->topography[node]) latddotA = (this->topography[node] - this->topography[to])/tdw * kd_lat * ddot;
			}

			// Same with laterl node B
			if(orthonodes.second >= 0 && orthonodes.second < this->connector->nnodes)
			{
				int& to = orthonodes.second;
				if(this->topography[to] > this->topography[node]) latedotB = (this->topography[to] - this->topography[node])/tdw * ke_lat * edot;
				if(this->topography[to] < this->topography[node]) latddotB = (this->topography[node] - this->topography[to])/tdw * kd_lat * ddot;
			}
			...
			if(quit)
			{
				std::cout << tau << "|" << edot << "|" << latedotA << "|" << latedotB<< "|" <<  ddot << "|" <<  latddotA << "|" << latddotB << "|" <<  tdx << "||" << tdw << "|" << this->qs[node] << std::endl;
			...
			{
				// regulator is then the max proportion of sediment that can be eaten by dep processes
			...
			// registering the versical motions to the current node (dep add z and e remove z)
			...
			if(orthonodes.second >= 0 && orthonodes.first < this->connector->nnodes)
			{
				vmot[orthonodes.second] += (latddotB - latedotB) * this->dt(node);
			}

			// qs receives what has been removed from land minus regulation and what remains
			this->qs[rec] += (edot + latedotA + latedotB - ddot - latddotA - latddotB) * tdx + this->qs[node];
			...
			if(std::isfinite((edot + latedotA + latedotB - ddot - latddotA - latddotB) * tdx) == false )
			{
				std::cout << tau << "||" << tau_c << "||" << edot  << "||" <<latedotA  << "||" << latedotB  << "||" << ddot  << "||" <<latddotA << "||" << latddotB << ":::" << regulator << "tt" << tdx << " ii " << tdw << std::endl;
	...
	void run_MFD_erosion(float_t ke, float_t aexp, float_t tau_c, float_t transport_length, float_t ke_lat, float_t alpha, float_t D, float_t rho_s)
	{
		return;
		// // Reinit the sediment flux
		// this->init_erosion();
		...
		// 		wsumslopes += (this->parting_coeff == 1) ? tsw : std::pow(tsw,parting_coeff);
		// 		recweight[j] = (this->parting_coeff == 1) ? tsw : std::pow(tsw,parting_coeff);;
		// 		sumslopes += tsw;
		// 	}
		...
		// 	// first deposition:
		// 	float_t d_dot = this->Qsin[node]/((this->connector->dx + this->connector->dy)/2 * transport_length);
		// 	if(d_dot * cA > this->Qsin[node])
		// 	{
		...
		// 			this->Qsout[node] += this->topological_number * edot * cA;
		...
		// 				{
		// 					float_t latedot = (this->topography[orthonodes.first] - this->topography[node])/tdl * edot * ke_lat;
		// 					latQ += this->topological_number * latedot * cA;
		// 					vmot[orthonodes.first] -= this->topological_number * latedot * this->dt(node);
		// 				}
		// 				else
		// 				{
		// 					float_t latddot = ((this->topography[node] - this->topography[orthonodes.first])/tdl)/(alpha * std::sqrt(tau / ((rho_s - RHO) * GRAVITY * D ) )) * this->Qsin[node] ;
		// 					vmot[orthonodes.first] += this->topological_number * latddot * this->dt(node);
		// 					latQ -= this->topological_number * latddot * cA;
		// 				}
		// 			}
		...
		// 				{
		// 					float_t latedot = (this->topography[orthonodes.second] - this->topography[node])/tdl * edot * ke_lat;
		// 					latQ += this->topological_number * latedot * cA;
		// 					vmot[orthonodes.second] -= this->topological_number * latedot * this->dt(node);
		// 				}
		// 				else
		// 				{
		// 					float_t latddot = ((this->topography[node] - this->topography[orthonodes.second])/tdl)/(alpha * std::sqrt(tau / ((rho_s - RHO) * GRAVITY * D ) )) * this->Qsin[node] ;
		// 					vmot[orthonodes.second] += this->topological_number * latddot * this->dt(node);
		// 					latQ -= this->topological_number * latddot * cA;
		// 				}
		// 			}
		...
		// 	vmot[node] += (this->Qsin[node] - this->Qsout[node])/cA * this->dt(node);
	...
	void run_MFD_erosion_B(float_t ke, float_t aexp, float_t tau_c, float_t transport_length, float_t ke_lat, float_t kd_lat)
	{
...
// 
	...
	{
		return (std::max(this->hw[n1] + this->topography[n1], this->hw[n2] + this->topography[n2]) - std::max(this->topography[n1] , this->topography[n2]));
	}
	

	float_t get_surface_at_node(int n1){return this->hw[n1] + this->topography[n1];}
	...
	// 	}
	// 	// std::cout << "Tot added by filling::" << totaddedbystuff << std::endl;
	...
	// 			this->Qwout[i] = this->graph->Sdistance2receivers[i]/this->mannings * std::pow(this->hw[i],5./3) * std::pow(gradients[i],1./2);
	// 			this->hw[i] += dt * ( this->Qwin[i] - this->Qwout[i])/this->connector->get_area_at_node(i);
	...
	// 			if(this->Qwout[i] > 0)
	// 			{
	// 				float_t E = K * std::pow((this->hw[i] + this->topography[i])/this->graph->Sdistance2receivers[i], n ) * std::pow(this->Qwout[i], m );
	// 				float_t D = this->qs[i]/(L * this->Qwout[i]);
	// 				this->qs[i] += E - D;
	...
	void basicFloodos(int n_prec, float_t base_dt, float_t dV, int n_noout)
	{
		...
		std::vector<float_t> gradhw(reclink.size(),0),sqrtgradhw(reclink.size(),0);
		...
		for(int i_prec=0; i_prec< n_prec; ++i_prec)
		{
			...
			int nwalk = 0;
			// -> running
			while(this->connector->is_active(i))
			{
				...
				// The following variables (with absolutely not instinctive names) are storing the information of the selected link for the precipiton's path
				// xam is the gradient, xamselect is the stochastic squareroot gradient to use for selecting the right gradient
				...
				// Calculating the effective timestep, corresponding to the time difference from the last precipiton path
				...
				for(int tn = 0; tn<nl; ++tn)
				{
					...
					if(this->topography[i] + this->hw[i] > this->topography[ri] + this->hw[ri])
					{
						...
						gradhw[nr] = (this->topography[i] + this->hw[i] - this->topography[ri] - this->hw[ri])/this->connector->get_dx_from_links_idx(li);
						// calculating the stochastic gradient
						sqrtgradhw[nr] = std::max(0., std::sqrt(gradhw[nr]) * this->randu.get()*8);
						// selecting the current neighbour as receiver to use for the walk if the stochastic gradient is higher
				...
				if(nr > 0)
				{
					...
					double tQout = this->connector->get_dx_from_links_idx(xi) * 1./this->mannings * std::pow(this->hw[i],double(5./3.)) * std::sqrt(xam);
					...
					{
						// the new hw is equal to the divergence of the fluxes (expressed as volume there)
						this->hw[i] += (dV - tQout* dt) /this->connector->get_area_at_node(i);
					}
		...
		for(int i_prec=0; i_prec< n_prec; ++i_prec)
		{
			...
			while(this->connector->is_active(i))
			{
				...
				for(int tn = 0; tn<nl; ++tn)
				{
					...
					float_t W_coef =  std::max(MANNING_COEF * std::sqrt(std::abs(last_S[ri]))*(1/this->mannings)/dx,1e-6);
				
					this->hw[ri] = this->hw[ri]*std::pow(1.0+W_coef*std::pow(this->hw[ri],MANNING_COEF)*tdt,-MANNING_COEF_INV);
					if(std::isfinite(this->hw[ri]) == false)
					{

						std::cout << W_coef << "," << last_S[ri] << "," << tdt << "," << std::endl;
				...
				float_t W_coef =  std::max(MANNING_COEF * std::sqrt(std::abs(last_S[i]))*(1/this->mannings)/dx,1e-6);
			
				this->hw[i] = this->hw[i]*std::pow(1.0+W_coef*std::pow(this->hw[i],MANNING_COEF)*tdt,-MANNING_COEF_INV);
				...
				for(int tn = 0; tn<nl; ++tn)
				{
					...
					// float_t exp = 0.66666;
					// float_t invexp = 1.5;
					// this->hw[ri] = this->hw[ri]*std::pow(1+coeff*std::pow(this->hw[ri],exp) * tdt,-invexp);
					...
					if(this->topography[i] + this->hw[i] > this->topography[ri] + this->hw[ri])
					{
						...
						gradhw[nr] = tgrad;
						// calculating the stochastic gradient
						sqrtgradhw[nr] = std::max(0., std::sqrt(gradhw[nr]) * this->randu.get()*1e-3);
						// selecting the current neighbour as receiver to use for the walk if the stochastic gradient is higher
						if(sqrtgradhw[nr] > xamselect)
						{
							// stochastic gradient
				...
				if(nr > 0)
				{
					...
					else
					{
						// the new hw is equal to the divergence of the fluxes (expressed as volume there)
						// this->hw[i] += (dV - tQout* dt) /this->connector->get_area_at_node(i);
						// float_t coeff = std::max(std::sqrt(last_S[i]) * this->mannings/this->connector->get_dx_from_links_idx(xi), 0.1);
						// // float_t C = 25;
						// float_t exp = 0.66666;
						// float_t invexp = 1.5;
						// float_t tdx = this->connector->get_dx_from_links_idx(xi);
						// if(dt > 0)
						// 	this->hw[i] = this->hw[i]*std::pow(1+coeff*std::pow(this->hw[i],exp) * dt,-invexp);
						// else
						this->hw[i] += (dV) /this->connector->get_area_at_node(i);
					}
					// removing negative discharges
	...
	void basicFloodos_v3(int n_prec, float_t base_dt, float_t precipitation_rates)
	{
		...
		// Calculating the Volume of precipitons function of uniform precipitation rates
		float_t dV = precipitation_rates * (base_dt) * this->graph->nnodes * this->connector->get_area_at_node(0);
		...
		for(int iprec=1; iprec<=n_prec; ++iprec)
		{
			...
			while(this->graph->is_active(i, *this->connector))
			{

				float_t dt = time - this->lastdt[i];
				if(this->lastdt[i] > 0 && this->lastdt[i] > 0 && std::abs(lastS[i]) > 0 )
					this->hw[i] = this->hw[i] * std::pow(1 + mannings_coeff * C / this->lastdl[i] * std::sqrt(std::abs(lastS[i])) * std::pow(this->hw[i],mannings_coeff) * dt, invmann);
				...
				{				
					...
					if(this->topography[i] + this->hw[i] > this->topography[ri] + this->hw[ri])
					{
						...
						gradhw[nr] = tgrad;
						// calculating the stochastic gradient
						sqrtgradhw[nr] = std::max(0., std::sqrt(gradhw[nr]) * this->randu.get());
						// selecting the current neighbour as receiver to use for the walk if the stochastic gradient is higher
						if(sqrtgradhw[nr] > xamselect)
						{
							// stochastic gradient
				...
				else
				{
					...
					while(true)
					{
						...
						if(hasrec || i == maxsurf_i) break;
						this->hw[i] = this->hw[maxsurf_i] + 1e-3 + this->topography[maxsurf_i] -  this->topography[i];
						i = maxsurf_i;
					}
				}
			}
	...
	void testDebugWalk(int n_prec, float_t base_dt, float_t K, float_t m, float_t n, float_t tolerance)
	{
		// last time passage of a single precipiton
		std::vector<float_t> last_t_precipitons(this->graph->nnodes,0.);

		std::vector<int> stack(this->graph->nnodes, -1), recs(this->graph->nnodes, -1);
		...
		for(int i_prec=0; i_prec< n_prec; ++i_prec)
		{
			...
			while(this->connector->is_active(i))
			{
				...
				for(int tn = 0; tn<nl; ++tn)
				{
					...
					{
						reclink[nr] = li;
						gradhw[nr] = (this->topography[i] - this->topography[ri])/this->connector->get_dx_from_links_idx(li);
				...
				if(nr>0)
				{
					// double E = 3e-5 * std::pow(xam,1) * std::pow(this->connector->get_area_at_node(i),0.45) * dt;
					// this->topography[i] -= E;
					stack[nn_proc] = i;
        ...
        while (std::abs(elevation_k - elevation_prev) > tolerance)
        {
					...
					float_t diff = ((elevation_k - node_elevation + factor * std::pow((slope),n)) / (1. + factor * n * std::pow(slope,(n - 1) ) ) );
					elevation_k -= diff;
        }
		...
		for(int i=0; i<this->graph->nnodes; ++i)
		{
			if(this->connector->is_active(i))
			{
				...
				for(int j=0; j<NN; ++j)
				{
					...
					float_t tS = (this->topography[i] + this->hw[i] - this->hw[ri] - this->topography[ri])/dri;
...
// Implementing Caesar-Lisflood flooding algorithm in DAGGER's logic
	...
	float_t caesar_lisflood_OMP()
	{
		...
		float_t dt = this->connector->dxmin * this->alpha/std::sqrt(GRAVITY * maxhw);
		...
		for(size_t i=0; i < this->Qlink.size(); ++i)
		{
			...
			// Get the trasverse dx
			float_t tdx = this->topological_number *this->connector->get_traverse_dx_from_links_idx(i);

			// Get the discharge
			this->Qlink[i] = tdx * (this->Qlink[i] - GRAVITY * hflow * dt * Sw) / ( 1 + (GRAVITY * hflow * dt * std::pow(this->mannings,2) * std::abs(this->Qlink[i])/std::pow(hflow,10./3.) ) );
			...
			if(std::abs(this->Qlink[i]) * dt / tdx > this->hw[n2]/n_neighbours && DAGGER::sgn(this->Qlink[i]) > 0 )
				this->Qlink[i] = this->hw[n2] * this->connector->get_area_at_node(n2)/(n_neighbours + 1)/dt;
			else if (std::abs(this->Qlink[i]) * dt / tdx > this->hw[n1]/n_neighbours && DAGGER::sgn(this->Qlink[i]) < 0 )
				this->Qlink[i] = -1 * this->hw[n1] * this->connector->get_area_at_node(n1)/(n_neighbours + 1)/dt;
