...
public:
	...
	void initialise_neighbourer()
	{
		...
		this->neighbourer.emplace_back(
			std::initializer_list<int>{ -this->nx,
																	this->not_a_node,
																	1,
		...
		this->neighbourer.emplace_back(
			std::initializer_list<int>{ (this->ny - 1) * this->nx,
																	this->nx - 1,
																	1,
	...
	int get_neighbour_idx_distance_linknodes(int i,
																					 std::vector<std::pair<int, T>>& out)
	{
		int n = 0;
		int next = this->get_right_idx_linknodes(i);
	...
	template<class ii_t>
	ii_t get_neighbour_idx_from_normalised_dxdy(ii_t node,
																							float_t normdx,
																							float_t normdy)
	{
																								...
																								bool ignore_nodata = false)
	{
		...
		this->set_neighbours_in_vector(
			neighbours, i, id_neighbourer, ignore_nodata);
	...
	template<class topo_t>
	void InitPriorityQue(topo_t& topography,
	...
	template<class topo_t>
	void ProcessTraceQue(topo_t& topography,
											 std::vector<bool>& flag,
	...
	template<class topo_t>
	void ProcessPit(topo_t& topography,
									std::vector<bool>& flag,
									std::queue<PQ_helper<int, double>>& depressionQue,
									std::queue<PQ_helper<int, double>>& traceQueue)
	{
