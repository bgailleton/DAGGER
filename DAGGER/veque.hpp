...
template<typename T,
				 typename ResizeTraits = fast_resize_traits,
...
public:
	...
	template<
		typename InputIt,
		typename ItCat = typename std::iterator_traits<InputIt>::iterator_category>
	...
	veque(const veque& other)
		: veque(
				_allocate_uninitialized_tag{},
				other.size(),
				alloc_traits::select_on_container_copy_construction(other._allocator()))
	{
		...
		: veque(alloc)
	{
		if constexpr (!alloc_traits::is_always_equal::value) {
			if (alloc != other._allocator()) {
				...
				auto replacement =
					veque(_allocate_uninitialized_tag{}, other.size(), alloc);
				_nothrow_move_construct_range(
					other.begin(), other.end(), replacement.begin());
	...
	reference emplace_front(Args&&... args)
	{
		...
		alloc_traits::construct(
			_allocator(), begin() - 1, std::forward<Args>(args)...);
		_move_begin(-1);
		return front();
	}
						 ...
						 alloc_traits::is_always_equal::value))
	{
		if constexpr (alloc_traits::propagate_on_container_swap::value) {
			...
			if (_allocator() == other._allocator()) {
				...
				_nothrow_move_construct_range(
					other.begin(), other.end(), new_this.begin());
...
private:
	...
	struct Data : Allocator
	{
		...
		Data(size_type size, const Allocator& alloc)
			: Allocator{ alloc }
			, _storage{ size ? std::allocator_traits<Allocator>::allocate(allocator(),
																																		size)
											 : nullptr }
			, _allocated{ size }
		{
	...
	veque(_allocate_uninitialized_tag,
				size_type size,
				size_type allocated,
				size_type offset,
				const Allocator& alloc)
	...
	template<typename... Args>
	void _value_construct_range(const_iterator b,
															const_iterator e,
															const Args&... args)
	{
	...
	{
		auto replacement = veque(
			_allocate_uninitialized_tag{}, size(), allocated, offset, _allocator());
	...
	iterator _insert_storage(const_iterator it, size_type count)
	{
		...
		_nothrow_move_construct_range(
			begin(), insertion_point, replacement.begin());
		_nothrow_move_construct_range(
			insertion_point, end(), replacement.begin() + index + count);
...
template<typename T,
				 typename LResizeTraits,
				 typename LAlloc,
				 typename RResizeTraits,
				 typename RAlloc>
inline bool
operator==(const veque<T, LResizeTraits, LAlloc>& lhs,
