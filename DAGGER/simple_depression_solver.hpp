...
namespace DAGGER
{
	...
	// ONLY CIRCULAR RINGS FITTING THE MEMORY COULD ADD SOME SPEED (i.e < few hundreds elements)
	template<class T>
	class vecQ
	{
	public:
	...
	out_t label_depressions_PQ(Connector_t* connector, in_t& ttopography)
	{
		...
		int ndep = 0;
		DBBTree tree;
		std::vector<int> labels = _label_depressions_PQ(connector, topography, ndep, tree);
	...
	std::vector<int> _label_depressions_PQ(Connector_t* connector, in_t& topography, int& ndep, DBBTree& tree )
	{
		// init label output
		...
		for(int i=0; i<connector->nnodes; ++i)
		{
			...
			if(connector->boundaries.no_data(i) || connector->flow_out_model(i)) continue;
		...
		while(sirius.empty() == false)
		{
			...
			if(tree.label[bas] == 2)
			{
				// if it is, it means this node is technically not in the depression anymore
				// if(next.node == 172627)
				// {
				// 	bool iswq = topography[next.node] == tree.outlet_Z[bas] ;
				// 	std::cout << "topo:" << topography[next.node] << " || outlet: " <<  tree.outlet_Z[bas]  << " || " << iswq<< std::endl;
				// 	throw std::runtime_error("kjfsdalk;jfsda");
				// }
				baslab[next.node] = 0;
				continue;
			}
			...
			for(int j=0; j<nn; ++j)
			{
				...
				else if((nbas == 0 && lowerz == false && inQ[nei] == false)) // || connector->boundaries.can_out(nei))
				{
	...
	std::vector<int> _label_ocean(Connector_t* connector, in_t& topography )
	{
		...
		// #ifdef BOOST_AVAILABLE
		// // std::queue<int, boost::circular_buffer<int> > nexus(boost::circular_buffer<int>(10000));
		...
		for(int i=0; i<connector->nnodes; ++i)
		{
			...
			else if (connector->flow_out_or_pit(i))
			{
				PiQ.emplace(i);
				ocean[i] = i;
				if(i == 150050) std::cout << "GASSS" << std::endl;
	...
	// 		{
	// 			int oi = neighbours[j];
	// 			if(connector->boundaries.no_data(oi) || ocean[oi] == 1 || topography[oi] < topography[next]) continue;
	// 			ocean[oi] = 1;
	...
	bool simple_depression_solver(Connector_t* connector, std::vector<fT>& topography, std::vector<size_t>& Sstack)
	{
		int ndep = 0;
		DBBTree tree;
		std::vector<int> baslab = _label_depressions_PQ(connector, topography, ndep, tree);
		...
		for(int bas = 1; bas<ndep; ++bas)
		{
			if(tree.top[bas] == bas)
			{
				...
				while(filler.empty() == false)
				{
					...
					{
						int nei = neighbours[j];
						if((baslab[nei] != bas && topography[nei] != tree.outlet_Z[bas]) || inQ[nei]) continue;
						inQ[nei] = true;
						topography[nei] = topo;
						// topo = topo + 1e-7 + connector->randu->get() * 1e-8;
						topo = std::nextafter(topo, std::numeric_limits<fT>::max());
		...
		// 			if((baslab[nei1] == targbas))// || (topography[nei1] <= minscore[targbas]))
		// 			{
		...
		// 			}
		// 			// else if(topography[nei1] <= topo && baslab[nei1] == 0)
		// 			// {
		// 			// 	bool newLM = true;
		// 			// 	int mm = connector->get_neighbour_idx(nei1, neighbours2);
