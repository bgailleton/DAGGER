...
// OLDAER TEST, UNFINISHED OR DO NOT WORK
	...
	// 		// 		throw std::runtime_error("Depression Hierarchy exception unhandled #342vf8");
	// 		// }
	// 		// if(valid)
	...
	// 			{
	// 				if(depression_labels[n1] == -1 && topography[n2] >= topography[n1])
	// 				{
	// 					mfdlab.emplace(n2);
	// 					depression_labels[n2] = -1;
	// 				}
	// 				else if(depression_labels[n2] == -1 && topography[n1] >= topography[n2])
	// 				{
	// 					mfdlab.emplace(n1);
	...
	// 		int nn = connector->get_neighbour_idx_links(next.node, neighbours);

	// 		// Is the cell already part of a depression?
	// 		if(depression_labels[next.node] == -2)
	// 		{
	// 			// if not I label it to new depressions (depression ID is the node id of the pit)
	// 			quik.emplace_back(depression(next.node,lab));
	...
	// 				int neigh = connector->get_other_node_from_links(lix,next.node);
	// 				if(depression_labels[neigh] < 0) continue;
	// 				// if a neighbour is a depression, this node is a potential outlet
	// 				bool valid = false;
	// 				if(topography[neigh] >= topography[next.node])
	// 					valid = quik[depression_labels[neigh]].external_link(lix, topography[neigh]);
	// 				if(valid) state[depression_labels[neigh]] = true;
	// 			}
	...
	// 		else
	// 		{
	// 			// If the depression is already connected to the ocean, I abort the process
	// 			if(state[depression_labels[next.node]])
	// 			{
