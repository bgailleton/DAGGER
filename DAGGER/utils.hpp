...
// Totally unsafe if hte priority queue has to be used again, but quite handy if you
// simply need to iterate through its elements
...
// Generate a random number between min and max (C style random umber generation)
// Quick and easy option, not the best for true randomness (e.g. with emscripten it always return the same series of random number for a given context)
template<class T>
...
template<class T>
void normalise_vector(std::vector<T>& vec)
{
...
// DAGGER relies heavily on templates to make its internal function usable from numpy arrays, R, js, c++...
// pvector process a vector into a pointer to it for this genericity
template<class T>
class pvector
{
public:
...
// Pretty self explanatory, T needs to be a vector-like data structure accessible via [] and having a size() attribute
template<class T, class U>
...
Note that I could use c++20 standard with constraints for that but it is a bit early
*/
...
/// Quick and easy lightweigth class to generate random numbers, by default between 0 and 1, and by extension to any range
class easyRand
{
public:
	std::random_device rd; // obtain a random number from hardware
