...
public:
	...
	void toposort_SFD_beta_1()
	{
		...
		reversedRadixSort(
			this->stack, this->stack.size() - tlevel, this->stack.size() - 1);
	...
	std::vector<float> _compute_drainage_area()
	{
		...
		for (int i = this->connector->nxy - 1; i > 7; i -= 8) {
			...
			ttAs = _mm256_setr_ps(tA[ino[0]],
														...
														tA[ino[6]],
														tA[ino[7]]);
			ttArecs = _mm256_setr_ps(tA[recs[0]],
															 tA[recs[1]],
															 tA[recs[2]],
															 tA[recs[3]],
															 tA[recs[4]],
	...
	std::vector<float> _compute_drainage_area_beta1()
	{
		...
		for (int i = this->connector->nxy - 1; i > 7; i -= 8) {
			...
			ttAs = _mm256_setr_ps(tA[i - 0],
														...
														tA[i - 6],
														tA[i - 7]);
			ttArecs = _mm256_setr_ps(tA[argrec[i - 0]],
															 tA[argrec[i - 1]],
															 tA[argrec[i - 2]],
															 tA[argrec[i - 3]],
															 tA[argrec[i - 4]],
	...
	void save_drainage_area(std::string filename)
	{
		...
		const std::vector<std::uint64_t> shape{
			std::uint64_t(this->connector->ny), std::uint64_t(this->connector->nx)
		};
