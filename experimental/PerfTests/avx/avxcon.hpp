...
template <class fT> class conavx {
public:
  ...
  void _neighbours_from_ui8_nocheck(int node, uint8_t idx,
                                    std::array<int, 8> &arr) {
    for (int i = 0; i < 8; ++i) {
      if (idx & (1 << i)) {
  ...
  __m256i _load_topoi(std::array<int, 8> &arrnodes) {
    return _mm256_set_epi32((arrnodes[0]), (arrnodes[1]), (arrnodes[2]),
                            (arrnodes[3]), (arrnodes[4]), (arrnodes[5]),
  ...
  __m256i _load_topoi(std::array<int, 8> &arrnodes, int tadder) {
    return _mm256_set_epi32((arrnodes[0] + tadder), (arrnodes[1] + tadder),
  ...
  __m256i _load_topoi(int i, std::vector<int> &vecnodes) {
    return _mm256_set_epi32((vecnodes[i + 0]), (vecnodes[i + 1]),
...
#pragma GCC unroll(8)
  ...
  void compute_beta1() {
    ...
    for (size_t i = 0; i < this->IntaNodes.size(); i += 8) {
      ...
      gatherIndices =
          _mm256_loadu_si256(reinterpret_cast<const __m256i *>(indicesPtr) +
                             static_cast<int>(i / 8));
...
#pragma GCC unroll(8)
  ...
  void _compute_edge_nodes() {
    ...
    for (size_t ti = 0; ti < this->EdgeNodes.size(); ++ti) {
      ...
      this->_neighbours_from_ui8_nocheck(i, this->EdgeNodes_Neighbours[ti],
                                         arr);
...
#pragma omp parallel for num_threads(4)
  ...
  template <class T> void save_vec(std::string filename, std::vector<T> &data) {
    const std::vector<std::uint64_t> shape{std::uint64_t(this->ny),
                                           std::uint64_t(this->nx)};
    const bool fortran_order{false};
    const std::string path{filename};
