...
constexpr uint32_t DonTopLeftMask =
	0b10000000000000000000000000000000; // woud be better to have 1 << 32
...
constexpr uint32_t SRecBottomLeftMask =
	0b00000000000000000000000000000100;																		// 11
constexpr uint32_t SRecBottomMask = 0b00000000000000000000000000000010; // 13
constexpr uint32_t SRecBottomRightMask =
	0b00000000000000000000000000000001; // 15

constexpr std::array<uint32_t, 8> DonMasks{
	DonTopLeftMask, DonTopMask,				 DonTopRightMask, DonLeftMask,
	DonRightMask,		DonBottomLeftMask, DonBottomMask,		DonBottomRightMask
};
constexpr std::array<uint32_t, 8> RecMasks{
	RecTopLeftMask, RecTopMask,				 RecTopRightMask, RecLeftMask,
	RecRightMask,		RecBottomLeftMask, RecBottomMask,		RecBottomRightMask
};
constexpr std::array<uint32_t, 8> SDonMasks{
	SDonTopLeftMask, SDonTopMask,				 SDonTopRightMask, SDonLeftMask,
	SDonRightMask,	 SDonBottomLeftMask, SDonBottomMask,	 SDonBottomRightMask
};
constexpr std::array<uint32_t, 8> SRecMasks{
	SRecTopLeftMask, SRecTopMask,				 SRecTopRightMask, SRecLeftMask,
	SRecRightMask,	 SRecBottomLeftMask, SRecBottomMask,	 SRecBottomRightMask
};
...
public:
	...
	// Adder is the regular "normal" neighbourer containing the value to add to
	// node i to get it's respective neighbours
	...
	conavx(int nx, int ny, fT dx, fT dy)
	{
		...
		this->_4edges();
		this->dxer = { this->dxy, this->dy,	 this->dxy, this->dx,
	...
	void _neighbours_from_ui8_nocheck(int node,
																		uint8_t idx,
																		std::array<int, 8>& arr)
	{
		for (int i = 0; i < 8; ++i) {
	...
	// void internal_neighbours_with_check(int node, std::array<int,8>&
	// neighbours)
	// {
	// 	int nn = 0;
	// 	for(int i=0;i<8;++i)
	...
	{
		return _mm256_set_ps(this->topography(node),
												 this->topography(node + 1),
												 this->topography(node + 2),
												 this->topography(node + 3),
												 this->topography(node + 4),
												 this->topography(node + 5),
	...
	{
		return _mm256_set_ps(this->topography(arrnodes[0]),
												 this->topography(arrnodes[1]),
												 this->topography(arrnodes[2]),
												 this->topography(arrnodes[3]),
												 this->topography(arrnodes[4]),
												 this->topography(arrnodes[5]),
												 this->topography(arrnodes[6]),
												 this->topography(arrnodes[7]));
	}

	__m256i _load_topoi(std::array<int, 8>& arrnodes)
	{
		return _mm256_set_epi32((arrnodes[0]),
														(arrnodes[1]),
														(arrnodes[2]),
														(arrnodes[3]),
														(arrnodes[4]),
	...
	{
		return _mm256_set_ps(this->topography(arrnodes[0] + tadder),
												 this->topography(arrnodes[1] + tadder),
												 this->topography(arrnodes[2] + tadder),
												 this->topography(arrnodes[3] + tadder),
												 this->topography(arrnodes[4] + tadder),
												 this->topography(arrnodes[5] + tadder),
												 this->topography(arrnodes[6] + tadder),
												 this->topography(arrnodes[7] + tadder));
	}

	__m256i _load_topoi(std::array<int, 8>& arrnodes, int tadder)
	{
		return _mm256_set_epi32((arrnodes[0] + tadder),
														(arrnodes[1] + tadder),
														(arrnodes[2] + tadder),
														(arrnodes[3] + tadder),
														(arrnodes[4] + tadder),
	...
	{
		return _mm256_set_ps(this->topography(vecnodes[i + 0]),
												 this->topography(vecnodes[i + 1]),
												 this->topography(vecnodes[i + 2]),
												 this->topography(vecnodes[i + 3]),
												 this->topography(vecnodes[i + 4]),
												 this->topography(vecnodes[i + 5]),
												 this->topography(vecnodes[i + 6]),
												 this->topography(vecnodes[i + 7]));
	}

	__m256i _load_topoi(int i, std::vector<int>& vecnodes)
	{
		return _mm256_set_epi32((vecnodes[i + 0]),
														(vecnodes[i + 1]),
														(vecnodes[i + 2]),
														(vecnodes[i + 3]),
														(vecnodes[i + 4]),
	...
	{
		return _mm256_set_ps(this->topography(vecnodes[i + 0] + tadder),
												 this->topography(vecnodes[i + 1] + tadder),
												 this->topography(vecnodes[i + 2] + tadder),
												 this->topography(vecnodes[i + 3] + tadder),
												 this->topography(vecnodes[i + 4] + tadder),
												 this->topography(vecnodes[i + 5] + tadder),
												 this->topography(vecnodes[i + 6] + tadder),
												 this->topography(vecnodes[i + 7] + tadder));
	}

	__m256i _load_topoi(int i, std::vector<int>& vecnodes, int tadder)
	{
		return _mm256_set_epi32((vecnodes[i + 0] + tadder),
														(vecnodes[i + 1] + tadder),
														(vecnodes[i + 2] + tadder),
														(vecnodes[i + 3] + tadder),
														(vecnodes[i + 4] + tadder),
	...
	{
		return _mm256_set_epi32(this->Neighbours[(node)],
														this->Neighbours[(node + 1)],
														this->Neighbours[(node + 2)],
														this->Neighbours[(node + 3)],
														this->Neighbours[(node + 4)],
														this->Neighbours[(node + 5)],
														this->Neighbours[(node + 6)],
														this->Neighbours[(node + 7)]);
	}
	__m256i _load_neighbours(std::array<int, 8>& arrnodes)
	{
		return _mm256_set_epi32(this->Neighbours[arrnodes[0]],
														this->Neighbours[arrnodes[1]],
														this->Neighbours[arrnodes[2]],
														this->Neighbours[arrnodes[3]],
														this->Neighbours[arrnodes[4]],
														this->Neighbours[arrnodes[5]],
														this->Neighbours[arrnodes[6]],
														this->Neighbours[arrnodes[7]]);
	}

	__m256i _load_neighbours(std::array<int, 8>& arrnodes, int tadder)
	{
		return _mm256_set_epi32(this->Neighbours[arrnodes[0] + tadder],
														this->Neighbours[arrnodes[1] + tadder],
														this->Neighbours[arrnodes[2] + tadder],
														this->Neighbours[arrnodes[3] + tadder],
														this->Neighbours[arrnodes[4] + tadder],
														this->Neighbours[arrnodes[5] + tadder],
														this->Neighbours[arrnodes[6] + tadder],
														this->Neighbours[arrnodes[7] + tadder]);
	}

	__m256i _load_neighbours(int i, std::vector<int>& vecnodes)
	{
		return _mm256_set_epi32(this->Neighbours[vecnodes[0 + i]],
														this->Neighbours[vecnodes[1 + i]],
														this->Neighbours[vecnodes[2 + i]],
														this->Neighbours[vecnodes[3 + i]],
														this->Neighbours[vecnodes[4 + i]],
														this->Neighbours[vecnodes[5 + i]],
														this->Neighbours[vecnodes[6 + i]],
														this->Neighbours[vecnodes[7 + i]]);
	}

	__m256i _load_neighbours(int i, std::vector<int>& vecnodes, int tadder)
	{
		return _mm256_set_epi32(this->Neighbours[vecnodes[0 + i] + tadder],
														this->Neighbours[vecnodes[1 + i] + tadder],
														this->Neighbours[vecnodes[2 + i] + tadder],
														this->Neighbours[vecnodes[3 + i] + tadder],
														this->Neighbours[vecnodes[4 + i] + tadder],
														this->Neighbours[vecnodes[5 + i] + tadder],
														this->Neighbours[vecnodes[6 + i] + tadder],
														this->Neighbours[vecnodes[7 + i] + tadder]);
	}
	...
	{
		return _mm256_set_epi32((-this->nx - 1 + node),
														(-this->nx - 1 + node + 1),
														(-this->nx - 1 + node + 2),
														(-this->nx - 1 + node + 3),
														(-this->nx - 1 + node + 4),
	...
	{
		return _mm256_set_epi32((-this->nx + node),
														(-this->nx + node + 1),
														(-this->nx + node + 2),
														(-this->nx + node + 3),
														(-this->nx + node + 4),
	...
	{
		return _mm256_set_ps(this->topography(-1 + node),
												 this->topography(-1 + node + 1),
												 this->topography(-1 + node + 2),
												 this->topography(-1 + node + 3),
												 this->topography(-1 + node + 4),
												 this->topography(-1 + node + 5),
												 this->topography(-1 + node + 6),
												 this->topography(-1 + node + 7));
	}

	__m256i _load_l_topoi(int node)
	{
		return _mm256_set_epi32((-1 + node),
														(-1 + node + 1),
														(-1 + node + 2),
														(-1 + node + 3),
														(-1 + node + 4),
	...
	void compute_archive_2()
	{
		...
		for (int i = this->nx + 1; i < this->nxy - this->nx - 1; i += 8) {
			...
			// comparisonMask = _mm256_not_si256(comparisonMask); // need to avoid
			// flats becominglinks Switching to
		...
		for (int i = this->nx + 1; i < this->nxy - this->nx - 1; i += 8) {
			...
			__m256i comparisonMask = _mm256_cmpeq_epi32(
				comparisonResult, bitmatg); //_mm256_and_si256(tneigh, bitmatg);
			...
			// _mm256_set1_epi32(_mm256_movemask_epi8(comparisonResult));
			// // // applying neighbour to temp vector
			...
			comparisonMask = _mm256_cmpeq_epi32(
				comparisonResult, bitmatg); //_mm256_and_si256(tneigh, bitmatg);
			...
			for (int j = 7; j >= 0; --j) {
				...
				// 	std::cout << std::bitset<8>(ui8fromui32(res[j],1)) <<
				// std::endl;; exit(0);
				// }
				++jj;
			}
...
#pragma GCC unroll(8)
			for (int j = 7; j >= 0; --j) {
				...
				// std::cout <<
				// std::bitset<8>(ui8fromui32(this->Neighbours[this->IntaNodes[i+jj]],0))
				// << "|";
				++jj;
			}
		}
...
#pragma GCC unroll(8)
			for (size_t j = 0; j < 8; ++j) {
				...
				comparisonMask = _mm256_cmpeq_epi32(
					comparisonResulti, bitmatg); //_mm256_and_si256(tneigh, bitmatg);
...
#pragma GCC unroll(8)
	...
	void compute_beta1()
	{
		...
		for (size_t i = 0; i < this->IntaNodes.size(); i += 8) {
			...
			gatherIndices = _mm256_loadu_si256(
				reinterpret_cast<const __m256i*>(indicesPtr) + static_cast<int>(i / 8));
...
#pragma GCC unroll(8)
	...
	void _compute_edge_nodes()
	{
		...
		for (size_t ti = 0; ti < this->EdgeNodes.size(); ++ti) {
			...
			this->_neighbours_from_ui8_nocheck(
				i, this->EdgeNodes_Neighbours[ti], arr);
	...
	void compute_classics()
	{
		...
		for (int i = 0; i < this->nxy; ++i) {
			...
			if (tSSi != i) {
				...
				// std::cout << std::bitset<32>(this->Neighbours[i])  << "|" <<
				// std::bitset<32>(this->Neighbours[tSSi]) << std::endl; std::cout <<
				// std::bitset<8>(ui8fromui32(this->Neighbours[i],0))  << "|" <<
				// std::bitset<8>(ui8fromui32(this->Neighbours[tSSi],1)) << std::endl;
			}
		}
		this->_compute_edge_nodes();
	}
...
#pragma omp parallel for num_threads(4)
	...
	// {
	// 	if(this->_topography == nullptr) throw std::runtime_error("no topo
	// linked sns");
	...
	void _compute_lookup_table()
	{
		for (int i = 0; i < LOOKUPSIZE; ++i) {
			...
			for (auto& v : this->lookup_table[ti])
				v = 0;

			// std::cout << std::bitset<8>(static_cast<uint8_t>(i)) << "|" <<
			...
			// std::cout << " (" << std::to_string(this->lookup_table[i][4]) << ")  "
			// << std::endl;;
		}
		// exit(0);
	}

	// Set of functions returning the 8 bits corresponding to the local neighbours
	// in a given topology

	uint8_t _get_ui8_rec(int node)
	{
		return ui8fromui32(this->Neighbours[node], 2);
	}
	uint8_t _get_ui8_Srec(int node)
	{
		return ui8fromui32(this->Neighbours[node], 0);
	}
	uint8_t _get_ui8_Don(int node)
	{
		return ui8fromui32(this->Neighbours[node], 3);
	}
	uint8_t _get_ui8_Sdon(int node)
	{
		return ui8fromui32(this->Neighbours[node], 1);
	}
	...
	void _4edges()
	{
		...
		for (int i = 0; i < this->nx; ++i) {
			...
			this->EdgeNodes_Neighbours.emplace_back((i == 0) ? 0b00001011
																							: (this->nx - 1 == i)
																								? 0b00010110
																								: 0b00011111);
		}
		...
		for (int i = this->nxy - this->nx; i < this->nxy; ++i) {
			this->boundaries[i] = 0;
			this->EdgeNodes.emplace_back(i);
			this->EdgeNodes_Neighbours.emplace_back(
				(i == this->nxy - this->nx) ? 0b01101000
				: (i == this->nxy - 1)			? 0b11010000
																		: 0b11111000);
		}
	...
	void _archive_compute_internal_nodes()
	{
		...
		// {
		// 	this->IntaNodes.emplace_back(std::array<int,8>{tempnodint[i +
		// 0],tempnodint[i + 1],tempnodint[i + 2],tempnodint[i + 3],tempnodint[i +
		// 4],tempnodint[i + 5],tempnodint[i + 6],tempnodint[i + 7]});
		// }
		...
		// for(int i=0; i<8; ++i)
		// {
		// 	temp2l[i] = (i<nmode) ? tempnodint[i + n_right_size] :
		// tempnodint[n_right_size - 1];
		// }
	...
	void save_topo(std::string filename)
	{
		...
		npy::SaveArrayAsNumpy(
			path, fortran_order, shape.size(), shape.data(), (*this->_topography));
	}
	...
	void save_neighbours(std::string filename)
	{
		...
		const std::string path{ filename };
		npy::SaveArrayAsNumpy(
			path, fortran_order, shape.size(), shape.data(), this->Neighbours);
	}
	...
	void save_Sreceivers(std::string filename)
	{
		...
		const std::string path{ filename };
		npy::SaveArrayAsNumpy(
			path, fortran_order, shape.size(), shape.data(), tvec);
	}
	...
	void save_vec(std::string filename, std::vector<T>& data)
	{
		...
		const std::string path{ filename };
		npy::SaveArrayAsNumpy(
			path, fortran_order, shape.size(), shape.data(), data);
	}
	...
	void _check_Sneighbouring()
	{
		...
		for (int i = 0; i < this->nxy; ++i) {
			...
			if (nn == 0) {
				...
				for (int j = 0; j < 8; ++j) {
					...
					if (this->topography(arr2[j]) > this->topography(i))
						std::cout << "JOIFSDDFLKJ::" << std::bitset<32>(this->Neighbours[i])
											<< "||" << std::bitset<32>(this->Neighbours[arr2[j]])
											<< std::endl;
				}
				// std::cout << std::bitset<32>(this->Neighbours[i])  << "|" <<
				// std::bitset<8>(ui8fromui32(this->Neighbours[i],1)) << std::endl;
			}
			...
			if (tr != i) {
				...
				if (tnd < 1) {
					std::cout << std::bitset<32>(this->Neighbours[i]) << "|"
										<< std::bitset<32>(this->Neighbours[tr]) << " | "
										<< (float(tr) / this->nx) << std::endl;
					++n_anomalies_Srec;
				}
			}
		}
		std::cout << "Out of " << this->nxy << " nodes checked " << n_sdon
							<< " and there was " << n_zerodon
							<< " with 0 donors. \nSDonors validity, anomaly founds: "
							<< n_anomalies
							<< "\nSrecs validity, anomaly founds: " << n_anomalies_Srec
							<< std::endl;
		;
	}
};
