...
blendvps_si256(__m256i& a, __m256i& b, __m256i& mask)
{
	__m256 res = _mm256_blendv_ps(
		_mm256_castsi256_ps(a), _mm256_castsi256_ps(b), _mm256_castsi256_ps(mask));
	return _mm256_castps_si256(res);
}

template<class Ti>
void
reorderVectors(const std::vector<Ti>& indices,
...
template<class Ti>
void
reorderVector_classics(const std::vector<Ti>& indices,
											 std::vector<float>& out,
...
template<class Ti>
void
reorderVector_classics_R_r(const std::vector<Ti>& indices,
													 std::vector<float>& out,
													 const std::vector<float>& inputVectors)
{
...
template<class Ti>
void
reorderVector_classics_R(const std::vector<Ti>& indices,
												 std::vector<float>& out,
												 const std::vector<float>& inputVectors)
{
	for (int i = indices.size() - 1; i >= 0; --i) {
...
template<class Ti>
void
reorderVector(const std::vector<Ti>& indices,
...
uint8_t
ui8fromui32(uint32_t value, int byteIndex)
{
...
void
AiisBiifCgtD(__m256i& A, __m256i& B, __m256& C, __m256& D)
{
...
__m256i
bitwiseORWithMask(__m256i& vec1, __m256i& vec2, __m256i& mask)
{
	__m256i maskedVec1 = _mm256_and_si256(vec1, mask);
	__m256i maskedVec2 =
		_mm256_and_si256(vec2, _mm256_xor_si256(mask, _mm256_set1_epi32(-1)));
	return _mm256_or_si256(maskedVec1, maskedVec2);
}

// from https://stackoverflow.com/a/42616203/7114716
inline __m256i
_mm256_not_si256(__m256i a)
{
	// return  _mm256_xor_si256 (a, _mm256_set1_epi32(0xffffffff));
	return _mm256_xor_si256(
		a, _mm256_cmpeq_epi32(a, a)); // I didn't check wich one is faster
}

// This function has been checked on the 5th of July
int
first_index_from_8bits(uint8_t indices, int default_return = -1)
{
...
void
printM256i_bitset_8(__m256i vec, int byteIndex)
{
	alignas(32) int temp[8];
	_mm256_storeu_si256((__m256i*)temp, vec);
...
uint32_t
getMax(const std::vector<uint32_t>& arr)
{
...
void
countingSort(std::vector<uint32_t>& arr, int exp)
{
...
void
radixSort(std::vector<uint32_t>& arr)
{
...
int
partition(std::vector<uint32_t>& arr, int low, int high)
{
...
void
quicksort(std::vector<uint32_t>& arr, int low, int high)
{
	if (low < high) {
...
uint32_t
getMax(const std::vector<uint32_t>& arr, int start, int end)
{
...
void
countingSort(std::vector<uint32_t>& arr, int start, int end, int exp)
{
...
void
radixSort(std::vector<uint32_t>& arr, int start, int end)
{
...
// Perform counting sort based on a specific digit
void
reversedRcountingSort(std::vector<uint32_t>& arr, int exp)
{
	const int n = arr.size();
	std::vector<uint32_t> output(n);
	std::vector<uint32_t> count(10, 0);
...
// Radix sort implementation
void
reversedRadixSort(std::vector<uint32_t>& arr)
{
	const uint32_t maxVal = getMax(arr);
...
// Perform counting sort based on a specific digit for a subsection of the array
void
reversedCountingSort(std::vector<uint32_t>& arr, int start, int end, int exp)
{
	const int n = end - start + 1;
	std::vector<uint32_t> output(n);
	std::vector<uint32_t> count(10, 0);
...
// Radix sort implementation for a subsection of the array
void
reversedRadixSort(std::vector<uint32_t>& arr, int start, int end)
{
	const uint32_t maxVal = getMax(arr, start, end);
