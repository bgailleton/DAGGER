...
public:
	...
	const std::vector<std::uint8_t> NeiMasks = { 0b10000000, 0b01000000,
	...
	{
		return _mm256_set_epi32((this->Sreceivers[node + 0]),
														(this->Sreceivers[node + 1]),
														(this->Sreceivers[node + 2]),
														(this->Sreceivers[node + 3]),
														(this->Sreceivers[node + 4]),
														(this->Sreceivers[node + 5]),
														(this->Sreceivers[node + 6]),
														(this->Sreceivers[node + 7]));
	}
	...
	void compute_sreceivers_avx_t1()
	{
		...
		for (int r = 1; r < this->ny - 1; ++r)
			for (int c = 1; c < this->nx - 1; ++c) {
				...
				__m256 topos = _mm256_set_ps(this->topography(neighbours[0]),
																		 this->topography(neighbours[1]),
																		 this->topography(neighbours[2]),
																		 this->topography(neighbours[3]),
																		 this->topography(neighbours[4]),
																		 this->topography(neighbours[5]),
																		 this->topography(neighbours[6]),
																		 this->topography(neighbours[7]));
...
#pragma omp parallel for num_threads(8)
	...
	void save_Sreceivers(std::string filename)
	{
		...
		const std::string path{ filename };
		npy::SaveArrayAsNumpy(
			path, fortran_order, shape.size(), shape.data(), this->Sreceivers);
	}
};
