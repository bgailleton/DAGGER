...
public:
	...
	const std::vector<std::uint8_t> NeiMasks = {0b10000000,0b01000000,0b00100000,0b00010000,0b00001000,0b00000100,0b00000010,0b00000001};
	const std::vector<int> NeiMasksint = {static_cast<int>(0b10000000),static_cast<int>(0b01000000),static_cast<int>(0b00100000),static_cast<int>(0b00010000),static_cast<int>(0b00001000),static_cast<int>(0b00000100),static_cast<int>(0b00000010),static_cast<int>(0b00000001)};
	...
	QD8(int nx, int ny, fT dx, fT dy)
	{
		...
		this->adder = {- this->nx - 1, - this->nx, - this->nx + 1, - 1, + 1, + this->nx - 1, + this->nx, + this->nx + 1};
	...
	{
		return _mm256_set_epi32(
			static_cast<int>(this->Sdonors[node + 0]),static_cast<int>(this->Sdonors[node + 1]), 
			static_cast<int>(this->Sdonors[node + 2]), static_cast<int>(this->Sdonors[node + 3]),
			static_cast<int>(this->Sdonors[node + 4]),static_cast<int>(this->Sdonors[node + 5]),
			static_cast<int>(this->Sdonors[node + 6]),static_cast<int>(this->Sdonors[node + 7])
		);
	}
	...
	void compute_sreceivers_sdonors()
	{
		...
		for(int c = 1; c<this->nx - 1; ++c)
		{
			...
			for(int j=0; j<8;++j)
			{
				int tn = neighbours[j];
				fT tSS = (this->topography(i) - this->topography(tn))/DXs[j];
				if(tSS > SS){SS = tSS;ri=tn;; tbmask = j;}
			}
			this->Sreceivers[i] = ri;
	...
	void compute_sreceivers_sdonors_i()
	{
		...
		for(int c = 1; c<this->nx - 1; ++c)
		{
			...
			for(int j=0; j<8;++j)
			{
				int tn = neighbours[j];
				fT tSS = (this->topography(i) - this->topography(tn))/DXs[j];
				if(tSS > SS){SS = tSS;ri=j;; tbmask = j;}
			}
			this->Sreceivers[i] = ri;
		}
	...
	void compute_sreceivers_sdonors_classic()
	{
		...
		for(int i=this->nx + 1; i< this->nxy - this->nx-1; ++i)
		{
			...
			if(i != Sreceivers[i])
			{
				...
				// this->classic_Sdonors[this->Sreceivers[i]*8 + this->classic_nSdonors[this->Sreceivers[i] ] ] = i ;
				// this->classic_Sdonors[this->Sreceivers[i]*8 + this->classic_nSdonors[this->Sreceivers[i] ] ] = babun[tmd] ;
				// // this->classic_Sdonors[this->Sreceivers[i]*8 + this->classic_nSdonors[this->Sreceivers[i] ] ] = i | this->NeiMasks[tbmask];
				// // this->classic_Sdonors[this->Sreceivers[i]*8 + this->classic_nSdonors[this->Sreceivers[i] ] ] = i | ri;
				// // this->classic_Sdonors[this->Sreceivers[i]*8 + this->classic_nSdonors[this->Sreceivers[i] ] ] = i | tmd;
				// // this->classic_Sdonors[this->Sreceivers[i]*8 + this->classic_nSdonors[this->Sreceivers[i] ] ] = i + ri;
		...
		// 	if(i != Sreceivers[i])
		// 	{
		// 		this->classic_Sdonors[this->Sreceivers[i]*8 + this->classic_nSdonors[i]] = (this->classic_Sdonors[i] | this->NeiMasksint[j]) ;
	...
	void compute_sreceivers_sdonors_classicv2()
	{
		...
		for(int i=this->nx + 1; i< this->nxy - this->nx-1; ++i)
		{
			...
			if(i != Sreceivers[i])
			{
				// this->Sdonors[this->Sreceivers[i]] = this->Sdonors[this->Sreceivers[i]] | tmd; 
				this->classic_Sdonors[this->Sreceivers[i]*8 + this->classic_nSdonors[this->Sreceivers[i] ] ] = i ;
				// this->classic_Sdonors[this->Sreceivers[i]*8 + this->classic_nSdonors[this->Sreceivers[i] ] ] = babun[tmd] ;
				// // this->classic_Sdonors[this->Sreceivers[i]*8 + this->classic_nSdonors[this->Sreceivers[i] ] ] = i | this->NeiMasks[tbmask];
				// // this->classic_Sdonors[this->Sreceivers[i]*8 + this->classic_nSdonors[this->Sreceivers[i] ] ] = i | ri;
	...
	void compute_sreceivers_avx_t1()
	{
		...
		for(int c = 1; c<this->nx - 1; ++c)
		{
			...
			for(int j=0; j<8;++j)
			{
				if(res[j] > SS ){SS = res[j];ri=neighbours[j];}
			}
			this->Sreceivers[i] = ri;
		}
	...
	void save_Sreceivers(std::string filename)
	{
		...
		const bool fortran_order{false};
		const std::string path{filename};
		npy::SaveArrayAsNumpy(path, fortran_order, shape.size(), shape.data(), this->Sreceivers);
	}
