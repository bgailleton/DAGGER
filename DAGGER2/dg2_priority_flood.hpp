...
private:
	/**
	 * Internal node state for priority queue processing
	 */
	struct FloodNode
	{
		size_t index;			// Grid index
		T elevation;			// Current elevation
		T fill_elevation; // Target fill elevation
		bool is_seed;			// Whether this is a boundary seed node

		FloodNode(size_t idx, T elev, T fill_elev, bool seed = false)
			: index(idx)
			, elevation(elev)
			, fill_elevation(fill_elev)
			, is_seed(seed)
		{
		}

		// Priority queue comparison (min-heap based on fill_elevation)
		bool operator>(const FloodNode& other) const
		{
			if (std::abs(fill_elevation - other.fill_elevation) <
					std::numeric_limits<T>::epsilon()) {
				return index > other.index; // Tie-breaking for deterministic results
			}
			return fill_elevation > other.fill_elevation;
		}
	};

	using PriorityQueue = std::
		priority_queue<FloodNode, std::vector<FloodNode>, std::greater<FloodNode>>;
...
public:
	 ...
	 * @param connector Connector for neighbor operations and boundary handling
																	...
																	const Config& config = Config())
	{
		...
		initialize_boundary_seeds(elevation, connector, pq, processed, results);
		...
		// Phase 2: Priority flood algorithm
		execute_priority_flood(
...
private:
	/**
	 * Initialize boundary seeds for the priority flood algorithm
	 */
	static void initialize_boundary_seeds(const Grid2D<T>& elevation,
																				const Connector<T>& connector,
																				PriorityQueue& pq,
																				...
																				Results& results)
	{
		...
		// Find all boundary nodes and nodes adjacent to NO_DATA
		for (size_t r = 0; r < rows; ++r) {
			for (size_t c = 0; c < cols; ++c) {
				size_t idx = connector.to_1d(r, c);
				NodeType node_type = connector.get_boundary_type(r, c);

				// Skip NO_DATA nodes entirely
				if (node_type == NodeType::NO_DATA) {
					processed[idx] = true; // Mark as processed to avoid revisiting
					continue;
				}
				...
				// // Check if this is a boundary seed node
				// if (connector.is_boundary_node(r, c)) {
				//     // Boundary nodes are seeds if they allow outflow or are outlets
				if (NodeTypeUtils::allows_outflow(node_type)) {
					is_boundary_seed = true;
				}
				// } else {
				//     // Internal nodes are seeds if they neighbor NO_DATA or domain
				//     boundary auto neighbors =
				//     connector.get_effective_valid_neighbors(r, c); for (const auto&
				//     neighbor : neighbors) {
				//         if (!neighbor.is_valid ||
				//             connector.get_boundary_type(neighbor.index) ==
				//             NodeType::NO_DATA) { is_boundary_seed = true; break;
				//         }
				//     }

				// // Also check if we're at the actual grid boundary
				// if (!is_boundary_seed && (r == 0 || r == rows-1 || c == 0 || c ==
				// cols-1)) {
				//     is_boundary_seed = true;
				// }
				// }
	...
	/**
	 * Execute the main priority flood algorithm
	 */
	static void execute_priority_flood(Grid2D<T>& elevation,
																		 const Connector<T>& connector,
																		 PriorityQueue& pq,
																		 std::vector<bool>& processed,
																		 const Config& config,
																		 Results& results)
	{
		...
		while (!pq.empty() && results.iterations < config.max_iterations) {
			...
			T original_elev = elevation[current.index];
			if (current.fill_elevation > original_elev) {
				T fill_depth = current.fill_elevation - original_elev;
				...
				elevation[current.index] = current.fill_elevation;
			...
			// Process neighbors
			auto [row, col] = connector.to_2d(current.index);
			...
			for (const auto& neighbor : neighbors) {
				...
				// Skip NO_DATA neighbors (should already be handled by
				// effective_valid_neighbors)
				if (neighbor.boundary_type == NodeType::NO_DATA) {
					continue;
				}

				T neighbor_elev = elevation[neighbor.index];
				T required_fill_elevation = current.fill_elevation;

				// Apply epsilon increment if needed
				if (neighbor_elev <= current.fill_elevation) {
					if (config.preserve_flat_areas &&
							std::abs(neighbor_elev - current.fill_elevation) <
								config.epsilon) {
						required_fill_elevation = current.fill_elevation;
					} else {
						required_fill_elevation = current.fill_elevation + config.epsilon;
					}
				} else {
				...
				pq.emplace(
					neighbor.index, neighbor_elev, required_fill_elevation, false);
